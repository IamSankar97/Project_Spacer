...
class SpacerEnv(btb.env.BaseEnv):
    def __init__(self, agent):
        ...
        #   (1)
        self.action_Musgrave = {'Scale': [1000, 50000]}
        #   (1)
        ...
        self.reset_action = {'specular': 0.2, 'ior': 2.3, 'roughness': 0.1, 'factor': 0.05, "value": 0.8,
                             'energy0': 0.01, 'ro_x': 0, 'ro_y': 0, 'Scale': 50000}
        self.action_bound = {**self.action_Material, **self.action_mix, **self.action_light, **self.action_Musgrave}
    ...
    def _env_reset(self):
        ...
        # if self.file_path_full is not None and os.path.exists(self.file_path_full):
        #     shutil.rmtree(self.file_path_full)
        self.episodes += 1
        self.total_step += 1
        self.step = 0
        ...
        self.update_musgrave(self.reset_action['Scale'])
    ...
    def _env_post_step(self, save_blend_file=False):
        ...
        img_file_path = self.file_path_full + '/image'
        ...
        if self.rl_training_mode:
            ...
            mask_file_path = self.file_path_full + '/mask'
    ...
    def take_action(self, actions):
        ...
        action_inverse_musgrave = self.inverse_normalization(self.action_Musgrave)
        self.update_musgrave(action_inverse_musgrave['Scale'])
        self.action_inverted = {**action_inverse_mat, **action_inverse_mix, **actions_inverse_specific_ligt,
                                **action_inverse_musgrave}
    ...
    def update_musgrave(self, Scale):
        mat = self.spacer.data.materials[0]
        if not mat.use_nodes:
            mat.use_nodes = True
        mat_nodes = mat.node_tree.nodes['Musgrave Texture']
        mat_nodes.inputs['Scale'].default_value = Scale
